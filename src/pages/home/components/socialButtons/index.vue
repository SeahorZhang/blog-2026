...
<template>
  <div v-if="show" class="absolute flex gap-2" :style="{ left: `${x}px`, top: `${y}px`, width: `${width}px` }">
    <motion.a
      v-for="(item, i) in socialLinks" :key="i" :initial="{ opacity: 0, scale: 0.6 }"
      :animate="{ opacity: 1, scale: 1 }" :while-hover="{ scale: 1.05 }" :transition="{
